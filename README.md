...
- **Keyboard Shortcut**: Trigger instant clean restart with <kbd>Ctrl</kbd> + <kbd>Alt</kbd> + <kbd>N</kbd> (or <kbd>Cmd</kbd> + <kbd>Alt</kbd> + <kbd>N</kbd> on macOS).
...
```mermaid
flowchart LR
    A[Click Status Bar / Shortcut / Sidebar] --> B[Stop Dev Server in Terminal]
    B --> C[Purge .next Directory]
    C --> D[Reuse Running Terminal]
...
2. **Stop**: Sends `Ctrl+C` to the dedicated terminal (default: `"Next.js Dev Server"`) and releases process handles.
3. **Purge**: Deletes `.next` using VS Code's file system API with automatic retry backoff.
4. **Restart**: Executes the dev script in the same running terminal (`pnpm dev`, `npm run dev`, etc.).
